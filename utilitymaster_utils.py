...
class VisualizeData():
    ...
    def visualize(self,train_loader):
        ...
        for images, labels in train_loader:
            ...
            plt.axis('off')
            plt.imshow(make_grid(images, nrow=30).permute(1, 2, 0).clamp(0,1))
            print('------')
            plt.show()
            print('------')
            break
...
class TrainTest():
    ...
    def test(self, model, device, test_loader, criterion):
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(test_loader):
                ...
                try:
                  ...
                  self.misclassified_pred_labels.extend(pred[misclassified_indices].squeeze().tolist())
                  self.misclassified_true_labels.extend(target[misclassified_indices].tolist())
                  self.misclassified_images.extend(data[misclassified_indices])
        ...
        return self.test_acc,self.test_losses,self.misclassified_images,self.misclassified_true_labels,self.misclassified_pred_labels
